...
public class RepositoryCreatedAt extends GitHubTagLibTagSupport {
	...
	String pattern = null;
	private static final Log log = LogFactory.getLog(RepositoryCreatedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theRepository.commitNeeded) {
				...
				if (theRepository.getCreatedAt() == null) {
					resultString = "";
				} else {
					if (pattern != null) {
						resultString = (new SimpleDateFormat(pattern)).format(theRepository.getCreatedAt());
					} else if (type.equals("BOTH")) {
						resultString = DateFormat.getDateTimeInstance(formatConvert(dateStyle),formatConvert(timeStyle)).format(theRepository.getCreatedAt());
					} else if (type.equals("TIME")) {
						resultString = DateFormat.getTimeInstance(formatConvert(timeStyle)).format(theRepository.getCreatedAt());
					} else { // date
						resultString = DateFormat.getDateInstance(formatConvert(dateStyle)).format(theRepository.getCreatedAt());
					}
				}
