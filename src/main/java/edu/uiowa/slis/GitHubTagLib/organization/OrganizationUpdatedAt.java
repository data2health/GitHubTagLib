...
public class OrganizationUpdatedAt extends GitHubTagLibTagSupport {
	...
	String pattern = null;
	private static final Log log = LogFactory.getLog(OrganizationUpdatedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theOrganization.commitNeeded) {
				...
				if (theOrganization.getUpdatedAt() == null) {
					resultString = "";
				} else {
					if (pattern != null) {
						resultString = (new SimpleDateFormat(pattern)).format(theOrganization.getUpdatedAt());
					} else if (type.equals("BOTH")) {
						resultString = DateFormat.getDateTimeInstance(formatConvert(dateStyle),formatConvert(timeStyle)).format(theOrganization.getUpdatedAt());
					} else if (type.equals("TIME")) {
						resultString = DateFormat.getTimeInstance(formatConvert(timeStyle)).format(theOrganization.getUpdatedAt());
					} else { // date
						resultString = DateFormat.getDateInstance(formatConvert(dateStyle)).format(theOrganization.getUpdatedAt());
					}
				}
