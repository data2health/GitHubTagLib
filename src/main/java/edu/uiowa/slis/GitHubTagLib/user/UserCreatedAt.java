...
public class UserCreatedAt extends GitHubTagLibTagSupport {
	...
	String pattern = null;
	private static final Log log = LogFactory.getLog(UserCreatedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theUser.commitNeeded) {
				...
				if (theUser.getCreatedAt() == null) {
					resultString = "";
				} else {
					if (pattern != null) {
						resultString = (new SimpleDateFormat(pattern)).format(theUser.getCreatedAt());
					} else if (type.equals("BOTH")) {
						resultString = DateFormat.getDateTimeInstance(formatConvert(dateStyle),formatConvert(timeStyle)).format(theUser.getCreatedAt());
					} else if (type.equals("TIME")) {
						resultString = DateFormat.getTimeInstance(formatConvert(timeStyle)).format(theUser.getCreatedAt());
					} else { // date
						resultString = DateFormat.getDateInstance(formatConvert(dateStyle)).format(theUser.getCreatedAt());
					}
				}
